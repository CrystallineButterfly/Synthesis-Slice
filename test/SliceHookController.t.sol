...
contract SliceHookControllerTest is Test {
    SliceHookController private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_slice");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new SliceHookController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        SliceHookController freshController = new SliceHookController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
