...
MODULE = importlib.import_module('agents.slice_bazaar')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'slice_bazaar')
        self.assertEqual(spec.primary_contract_name, 'SliceHookController')
